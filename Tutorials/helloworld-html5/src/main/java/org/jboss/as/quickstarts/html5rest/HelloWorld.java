...
public class HelloWorld {
    ...
    @Path("/json/{name2}")
    @Produces("application/json")
    public String getHelloWorldJSON2(@PathParam("name2") String name2) {
        System.out.println("name2: " + name2);
        return "{\"result\":\"" + helloService.createHelloMessage(name2) + "\"}";
    }

    @POST
    @Path("/xml/{name2}")
    @Produces("application/xml")
    public String getHelloWorldXML2(@PathParam("name2") String name2) {
        System.out.println("name: " + name2);
        return "<xml><result>" + helloService.createHelloMessage(name2) + "</result></xml>";
    }
